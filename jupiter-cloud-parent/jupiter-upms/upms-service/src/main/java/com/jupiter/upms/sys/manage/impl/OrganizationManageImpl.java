...
public class OrganizationManageImpl extends GenericManageImpl<OrganizationDao, Organization>
    ...
    public boolean insert(Organization param) {
        ...
        if (StringUtils.isNotBlank(org.getParentCode())) {
            Organization parent = this
                .selectOne(new EntityWrapper<>(new Organization(org.getParentCode(), org.getTenantId())));
            if (parent == null)
                throw new BusinessException(UpmsExceptionCodeEnum.ORGANIZATION_NOT_EXISTS,
                    org.getParentCode());
            org.setLevel(parent.getLevel() + 1);

            Wrapper<Organization> wrapper =
                    new EntityWrapper<>(new Organization(org.getTenantId(), org.getParentCode()));
            int count = this.selectCount(wrapper);
            org.setCode(String.format("%s%02d", org.getParentCode(), count + 1));

        } else {
            org.setLevel(1);
            org.setParentCode("");

            Wrapper<Organization> wrapper = new EntityWrapper<>(new Organization());
            wrapper.getEntity().setTenantId(param.getTenantId());
            int count = this.selectCount(wrapper);
            org.setCode(String.format("%02d", count + 1));
    ...
    public List<OrganizationTreeVo> trees(Long tenantId) {
        ...
        tenantMap.entrySet().forEach(entry -> {
            Map<String, List<Organization>> map =
                    entry.getValue().stream().collect(Collectors.groupingBy(Organization::getParentCode));

            List<Organization> top =
                    entry.getValue().stream().filter(a -> a.getLevel() == 1).collect(Collectors.toList());

            top.forEach(o -> {
                result.add(this.convert(o, map));
            });
        });
