	...
	public Swagger mapDocumentation(Documentation from) {
		...
		for (Entry<String, Path> entry : swagger.getPaths().entrySet()) {
			...
			if (entry.getValue().getPost() != null) {
				response = entry.getValue().getPost().getResponses().get(Integer.toString(HttpStatus.OK.value()));
			} else if (entry.getValue().getGet() != null) {
				response = entry.getValue().getGet().getResponses().get(Integer.toString(HttpStatus.OK.value()));
			} else {
				continue;
			}
